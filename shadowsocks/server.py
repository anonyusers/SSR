...
def main():
    ...
    daemon.daemon_exec(config) # 若指定daemon,则在后台执行
    ...
    if config['port_password']:
        ...
        config['port_password'] = {} # 服务端开启多个端口
        server_port = config['server_port']
        if type(server_port) == list:
            for a_server_port in server_port:
                config['port_password'][a_server_port] = config['password'] # 按端口划分服务器
        else:
    ...
    tcp_servers = []
    udp_servers = []
    dns_resolver = asyncdns.DNSResolver() # 获取dns解析器
    ...
    for port, password_obfs in port_password.items():
        ...
        protocol = config.get("protocol", 'origin') # 从配置中获取协议项
        protocol_param = config.get("protocol_param", '')
        obfs = config.get("obfs", 'plain') # 从配置中获取obfs插件项
        obfs_param = config.get("obfs_param", '')
        bind = config.get("out_bind", '')
        bindv6 = config.get("out_bindv6", '')
        if type(password_obfs) == list: # pasword_obfs可组合密码,obfs插件,协议
