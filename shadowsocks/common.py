...
_ord = ord # 返回字符的Unicode编码
_chr = chr # 返回Unicode编码的字符串形式
ord = compat_ord
chr = compat_chr
...
def inet_ntop(family, ipstr):
    # 转换字节表示的ip地址为点分十进制字符串形式，支持AF_INET和AF_INET6协议族
    if family == socket.AF_INET: # ipv4协议族
        return to_bytes(socket.inet_ntoa(ipstr))
    elif family == socket.AF_INET6: # ipv6协议族
        import re
...
def inet_pton(family, addr):
    # 转换字符串形式的ip地址为二进制字节形式，支持AF_INET和AF_INET6协议族
    addr = to_str(addr)
